  ...
  try {
    ...
    if (rerankResponse.ok && rerankResponse.data) {
      ...
      if (rerankData.results && rerankData.results.length > 0) {
        ...
        log(
          colors.blue,
          `    Top result: "${rerankData.results[0].document}" (score: ${rerankData.results[0].relevance_score.toFixed(4)})`,
        );
      } else {
    ...
    for (const query of renderQueries) {
      ...
      if (rerankResponse.ok && rerankResponse.data) {
        ...
        if (results && results.length > 0) {
          ...
          log(
            colors.green,
            `    ✅ Top match: ${topTool.document.split(' - ')[0]} (score: ${topTool.relevance_score.toFixed(4)})`,
          );
        } else {
